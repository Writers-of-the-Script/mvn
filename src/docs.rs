...
use crate::{cx::RouteContext, err::AxumResponse};
...
pub async fn docs_handler(state: RouteContext, parts: Vec<String>) -> Result<Response, Response> {
    match parse_parts(parts) {
        Ok(DocsPathParts {
            ...
            let jar_file = lock.get_file(&jar_path).await.into_axum()?;

            let jar = jar_file.get_bytes(&lock.storage).await.into_axum()?;
