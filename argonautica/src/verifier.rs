...
use crate::config::{Backend, VerifierConfig, default_runtime};
...
impl<'a> Verifier<'a> {
    ...
    pub fn configure_runtime(&mut self, runtime: Handle) -> &mut Verifier<'a> {
        self.hasher.config.set_runtime(runtime);
        self
    }
    ...
    pub fn verify_non_blocking(&mut self) -> Pin<Box<dyn Future<Output = Result<bool, Error>>>> {
        let mut verifier = self.to_owned();
        match verifier.hasher.config.runtime() {
            Some(rt) => Box::pin(
                rt.spawn_blocking(move || verifier.verify())
                    .map(|it| it.unwrap_or(Ok(false))),
            ),
            None => {
                let rt = default_runtime();
                verifier.hasher.config.set_runtime(rt.clone());
                Box::pin(
                    rt.spawn_blocking(move || verifier.verify())
                        .map(|it| it.unwrap_or(Ok(false))),
                )
    ...
    pub fn config(&self) -> VerifierConfig {
        VerifierConfig::new(
            ...
            /* runtime */ self.hasher.config.runtime(),
