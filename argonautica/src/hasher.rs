...
use crate::config::defaults::default_lanes;
use crate::config::{Backend, HasherConfig, Variant, Version, default_runtime};
...
impl<'a> Hasher<'a> {
    ...
    pub fn configure_runtime(&mut self, runtime: Handle) -> &mut Hasher<'a> {
    ...
    pub fn hash_non_blocking(&mut self) -> impl Future<Output = Result<String, Error>> {
        async {
            self.hash_raw_non_blocking().await.and_then(|hash_raw| {
                let hash = hash_raw.encode_rust();
                Ok::<_, Error>(hash)
            })
        }
    ...
    /// [`Future`](https://docs.rs/futures/0.1.21/futures/future/trait.Future.html)
    /// instead of a [`Result`](https://doc.rust-lang.org/std/result/enum.Result.html)
    pub fn hash_raw_non_blocking(&mut self) -> Pin<Box<dyn Future<Output = Result<HashRaw, Error>>>> {
        let hasher = scopeguard::guard(self, |hasher| {
            hasher.clear();
        });
        let mut hasher = hasher.to_owned();
        match hasher.config.runtime() {
            Some(rt) => Box::pin(
                rt.spawn_blocking(move || hasher.hash_raw())
                    .map(|it| it.unwrap()),
            ),
            None => {
                let rt = default_runtime();
                hasher.config.set_runtime(rt.clone());
                Box::pin(
                    rt.spawn_blocking(move || hasher.hash_raw())
                        .map(|it| it.unwrap()),
                )
